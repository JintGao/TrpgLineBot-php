...
function Rules_Operating($inputStr,$userName) { 
	    ...
	    foreach ($data['feed']['entry'] as $item) 
	    {
	    	if($userName == $item['gsx$玩家名稱']['$t'])
	    	{
				...
				else if($inputStr=='判A'||$inputStr=='判1' || $inputStr=='招式1' 
					                    || $inputStr==$data['feed']['entry'][$序號]['gsx$A攻擊招式']['$t'] )
				{
				     return nomalDiceRoller_Customize("2d6".$data['feed']['entry'][$序號]['gsx$A攻擊數值']['$t'],$inputStr,$player);
				}				
				else if($inputStr=='判B'||$inputStr=='判2' || $inputStr=='招式2' 
					                    || $inputStr==$data['feed']['entry'][$序號]['gsx$B攻擊招式']['$t'] )
				{
				     return nomalDiceRoller_Customize("2d6".$data['feed']['entry'][$序號]['gsx$B攻擊數值']['$t'],$inputStr,$player);
				}
				else if($inputStr=='判C'||$inputStr=='判3' || $inputStr=='招式3' 
					                    || $inputStr==$data['feed']['entry'][$序號]['gsx$C攻擊招式']['$t'] )
				{
